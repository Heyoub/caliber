...
use crate::heap_ops::{
    current_timestamp, delete_tuple, form_tuple, get_active_snapshot, insert_tuple, open_relation,
    timestamp_to_pgrx, HeapRelation, HeapScanner, PgLockMode as HeapLockMode,
};
use crate::index_ops::{
...
use crate::tuple_extract::{
    chrono_to_timestamp, extract_text, extract_timestamp, extract_uuid, extract_values_and_nulls,
...
pub fn lock_acquire_heap(
    ...
    let expires_datum = chrono_to_timestamp(expires_at)?
        .into_datum()
        .ok_or_else(|| {
            CaliberError::Storage(StorageError::InsertFailed {
                entity_type: EntityType::Lock,
...
pub fn lock_list_by_resource_heap(
    ...
    let mut scanner =
        unsafe { IndexScanner::new(&rel, &index_rel, snapshot, 2, scan_keys.as_mut_ptr()) };
...
unsafe fn tuple_to_lock(
    ...
    let lock_id = extract_uuid(tuple, tuple_desc, lock::LOCK_ID)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
            reason: "lock_id is NULL".to_string(),
        })
    })?;

    let resource_type = extract_text(tuple, tuple_desc, lock::RESOURCE_TYPE)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
            reason: "resource_type is NULL".to_string(),
        })
    })?;

    let resource_id = extract_uuid(tuple, tuple_desc, lock::RESOURCE_ID)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let holder_agent_id =
        extract_uuid(tuple, tuple_desc, lock::HOLDER_AGENT_ID)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let acquired_at_ts =
        extract_timestamp(tuple, tuple_desc, lock::ACQUIRED_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let expires_at_ts =
        extract_timestamp(tuple, tuple_desc, lock::EXPIRES_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let mode_str = extract_text(tuple, tuple_desc, lock::MODE)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let tenant_id = extract_uuid(tuple, tuple_desc, lock::TENANT_ID)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
...
mod tests {
    ...
    fn arb_lock_mode() -> impl Strategy<Value = LockMode> {
        prop_oneof![Just(LockMode::Exclusive), Just(LockMode::Shared),]
    }

    /// Generate a future expiration time (1-60 seconds from now)
    fn arb_expires_at() -> impl Strategy<Value = chrono::DateTime<chrono::Utc>> {
        (1i64..60i64).prop_map(|seconds| chrono::Utc::now() + Duration::seconds(seconds))
    }
    ...
    mod pg_tests {
        ...
        fn prop_lock_insert_get_roundtrip() {
            ...
            runner
                .run(
                    &strategy,
                    |(resource_type, resource_id, holder_agent_id, expires_at, mode)| {
            ...
            runner
                .run(&any::<[u8; 16]>(), |bytes| {
                    ...
                    prop_assert!(
                        ...
                        "Non-existent lock should return None"
                    );
                ...
                .run(
                    ...
                    |(resource_type, resource_id, holder_agent_id, expires_at, mode)| {
                        ...
                        prop_assert!(
                            ...
                            "Lock should exist before delete"
                        );
        ...
        fn prop_lock_delete_nonexistent_returns_false() {
            ...
            runner
                .run(&any::<[u8; 16]>(), |bytes| {
                    ...
                    prop_assert!(
                        !result.unwrap(),
                        "Delete of non-existent lock should return false"
                    );
            ...
            runner
                .run(
                    ...
                    |(resource_type, resource_id, holder_agent_id, expires_at, mode)| {
                        ...
                        prop_assert!(
                            list_result.is_ok(),
                            "List by resource should succeed: {:?}",
                            list_result.err()
                        );
                        ...
                        let found_lock = locks
                            .iter()
                            .find(|l| l.lock.lock_id.as_uuid() == lock_id.as_uuid())
                            .unwrap();
                        prop_assert_eq!(&found_lock.lock.resource_type, &resource_type);
                        prop_assert_eq!(found_lock.lock.resource_id, resource_id);
                        prop_assert_eq!(
                            found_lock.lock.holder_agent_id.as_uuid(),
                            holder_agent_id.as_uuid()
                        );
