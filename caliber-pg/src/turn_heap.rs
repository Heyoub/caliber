...
use caliber_core::{
    CaliberError, CaliberResult, EntityIdType, EntityType, ScopeId, StorageError, TenantId, Turn,
    TurnId, TurnRole,
};
...
use crate::tuple_extract::{
    extract_i32, extract_jsonb, extract_text, extract_timestamp, extract_uuid, i32_to_datum,
    json_to_datum, string_to_datum, timestamp_to_chrono, uuid_to_datum,
};
...
pub fn turn_get_by_scope_heap(
    scope_id: ScopeId,
    tenant_id: TenantId,
) -> CaliberResult<Vec<TurnRow>> {
...
unsafe fn tuple_to_turn(
    ...
    let turn_id = extract_uuid(tuple, tuple_desc, turn::TURN_ID)?.ok_or_else(|| {
    ...
    let scope_id = extract_uuid(tuple, tuple_desc, turn::SCOPE_ID)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let sequence = extract_i32(tuple, tuple_desc, turn::SEQUENCE)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
            reason: "sequence is NULL".to_string(),
        })
    })?;

    let role_str = extract_text(tuple, tuple_desc, turn::ROLE)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let content = extract_text(tuple, tuple_desc, turn::CONTENT)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
            reason: "content is NULL".to_string(),
        })
    })?;

    let token_count = extract_i32(tuple, tuple_desc, turn::TOKEN_COUNT)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let created_at_ts =
        extract_timestamp(tuple, tuple_desc, turn::CREATED_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
...
mod tests {
    ...
    mod pg_tests {
        ...
        fn prop_turn_insert_get_roundtrip() {
            ...
            let strategy = (arb_content(), arb_role(), arb_token_count(), arb_sequence());

            runner
                .run(&strategy, |(content, role, token_count, sequence)| {
                ...
                .run(&strategy, |num_turns| {
                    ...
                    for (i, turn) in turns.iter().enumerate() {
                        prop_assert_eq!(
                            turn.turn.sequence,
                            i as i32,
                            "Turns should be ordered by sequence"
                        );
                        prop_assert_eq!(
                            turn.tenant_id.map(|t| t.as_uuid()),
            ...
            runner
                .run(&any::<[u8; 16]>(), |bytes| {
                    ...
                    prop_assert!(
                        ...
                        "Get for non-existent scope should be empty"
                    );
