...
use crate::tuple_extract::{
    extract_text, extract_text_array, extract_timestamp, extract_uuid, extract_values_and_nulls,
    string_to_datum, text_array_to_datum, timestamp_to_chrono, uuid_to_datum,
};
...
pub fn handoff_create_heap(params: HandoffCreateParams<'_>) -> CaliberResult<HandoffId> {
    ...
    let mut values: [pg_sys::Datum; handoff::NUM_COLS] =
        [pg_sys::Datum::from(0); handoff::NUM_COLS];
    ...
    values[handoff::CONTEXT_SNAPSHOT_ID as usize - 1] =
        uuid_to_datum(context_snapshot_id.as_uuid());
...
pub fn handoff_get_heap(
    handoff_id: HandoffId,
    tenant_id: TenantId,
) -> CaliberResult<Option<HandoffRow>> {
...
unsafe fn tuple_to_handoff(
    ...
    let handoff_id = extract_uuid(tuple, tuple_desc, handoff::HANDOFF_ID)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let from_agent_id =
        extract_uuid(tuple, tuple_desc, handoff::FROM_AGENT_ID)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let trajectory_id =
        extract_uuid(tuple, tuple_desc, handoff::TRAJECTORY_ID)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
                reason: "trajectory_id is NULL".to_string(),
            })
        })?;

    let scope_id = extract_uuid(tuple, tuple_desc, handoff::SCOPE_ID)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let handoff_notes =
        extract_text(tuple, tuple_desc, handoff::HANDOFF_NOTES)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let next_steps =
        extract_text_array(tuple, tuple_desc, handoff::NEXT_STEPS)?.unwrap_or_default();

    let blockers = extract_text_array(tuple, tuple_desc, handoff::BLOCKERS)?.unwrap_or_default();

    let open_questions =
        extract_text_array(tuple, tuple_desc, handoff::OPEN_QUESTIONS)?.unwrap_or_default();

    let status_str = extract_text(tuple, tuple_desc, handoff::STATUS)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let status = match status_str.as_str() {
        ...
        _ => {
            pgrx::warning!(
                "CALIBER: Unknown handoff status '{}', defaulting to Initiated",
                status_str
            );
    ...
    let initiated_at_ts =
        extract_timestamp(tuple, tuple_desc, handoff::INITIATED_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let accepted_at =
        extract_timestamp(tuple, tuple_desc, handoff::ACCEPTED_AT)?.map(timestamp_to_chrono);

    let completed_at =
        extract_timestamp(tuple, tuple_desc, handoff::COMPLETED_AT)?.map(timestamp_to_chrono);

    let reason_str = extract_text(tuple, tuple_desc, handoff::REASON)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
            ...
            runner
                .run(
                    ...
                    |(
                        ...
                        prop_assert_eq!(
                            h.to_agent_id.map(|a| a.as_uuid()),
                            to_agent_id.map(|a| a.as_uuid())
                        );
                        ...
                        prop_assert_eq!(
                            h.context_snapshot_id.map(|id| id.as_uuid()),
                            Some(context_snapshot_id.as_uuid())
                        );
                        ...
                        prop_assert!(
                            h.accepted_at.is_none(),
                            "accepted_at should be None initially"
                        );
                        prop_assert!(
                            h.completed_at.is_none(),
            ...
            runner
                .run(&any::<[u8; 16]>(), |bytes| {
                    ...
                    prop_assert!(
                        ...
                        "Non-existent handoff should return None"
                    );
            ...
            runner
                .run(
                    ...
                    |(
                        ...
                        prop_assert!(
                            handoff_before.accepted_at.is_none(),
                        ...
                        let accept_result =
                            handoff_accept_heap(handoff_id, accepting_agent, tenant_id);
                        prop_assert!(
                            accept_result.is_ok(),
                        ...
                        prop_assert!(
                            ...
                            "Accept should return true for existing handoff"
                        );
                        ...
                        prop_assert!(
                            handoff_after.accepted_at.is_some(),
                            "accepted_at should be set after accept"
                        );
                        prop_assert!(
                            handoff_after.accepted_at.unwrap() <= chrono::Utc::now(),
                            "accepted_at should be <= now"
                        );
                        prop_assert_eq!(
                            handoff_after.to_agent_id.map(|a| a.as_uuid()),
                            Some(accepting_agent.as_uuid()),
                            "to_agent_id should be set to accepting agent"
                        );
            ...
            runner
                .run(
                    ...
                    |(
                        ...
                        prop_assert_eq!(handoff_before.status, HandoffStatus::Initiated);
                        prop_assert!(
                            handoff_before.completed_at.is_none(),
                        ...
                        prop_assert!(
                            ...
                            "Complete should return true for existing handoff"
                        );
                        ...
                        prop_assert!(
                            handoff_after.completed_at.is_some(),
                            "completed_at should be set after complete"
                        );
                        prop_assert!(
                            handoff_after.completed_at.unwrap() <= chrono::Utc::now(),
            ...
            runner
                .run(
                    ...
                    |(bytes, agent_bytes)| {
                        ...
                        let tenant_id = TenantId::now_v7();
                        let accept_result =
                            handoff_accept_heap(random_id, random_agent_id, tenant_id);
                        prop_assert!(accept_result.is_ok(), "Accept should not error");
                        prop_assert!(
                            !accept_result.unwrap(),
                            "Accept of non-existent handoff should return false"
                        );
                        ...
                        prop_assert!(
                            !complete_result.unwrap(),
                            "Complete of non-existent handoff should return false"
                        );
