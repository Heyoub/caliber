...
use crate::tuple_extract::{
    content_hash_to_datum, extract_content_hash, extract_float4_array, extract_i32, extract_jsonb,
    extract_text, extract_timestamp, extract_uuid, extract_uuid_array, extract_values_and_nulls,
    float4_array_to_datum, i32_to_datum, json_to_datum, string_to_datum, timestamp_to_chrono,
    uuid_array_to_datum, uuid_to_datum,
};
...
pub fn note_create_heap(params: NoteCreateParams<'_>) -> CaliberResult<NoteId> {
    ...
    if !source_trajectory_ids.is_empty() {
        let trajectory_uuids: Vec<uuid::Uuid> = source_trajectory_ids
            .iter()
            .map(|id| id.as_uuid())
            .collect();
    ...
    if !source_artifact_ids.is_empty() {
        let artifact_uuids: Vec<uuid::Uuid> =
            source_artifact_ids.iter().map(|id| id.as_uuid()).collect();
    ...
    values[note::ABSTRACTION_LEVEL as usize - 1] =
        string_to_datum(abstraction_level_to_str(abstraction_level));
...
pub fn note_query_by_trajectory_heap(
    ...
    let mut scanner =
        unsafe { crate::heap_ops::HeapScanner::new(&rel, snapshot, 0, std::ptr::null_mut()) };
    ...
    for tuple in &mut scanner {
        let source_ids =
            unsafe { extract_uuid_array(tuple, tuple_desc, note::SOURCE_TRAJECTORY_IDS) }?;
...
pub fn note_update_heap(
    ...
    let now = current_timestamp();
    values[note::UPDATED_AT as usize - 1] =
...
fn str_to_abstraction_level(s: &str) -> AbstractionLevel {
    match s {
        ...
        _ => {
            pgrx::warning!(
                "CALIBER: Unknown abstraction level '{}', defaulting to Raw",
                s
            );
...
unsafe fn tuple_to_note(
    ...
    let note_id = extract_uuid(tuple, tuple_desc, note::NOTE_ID)?.ok_or_else(|| {
    ...
    let note_type_str = extract_text(tuple, tuple_desc, note::NOTE_TYPE)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let title = extract_text(tuple, tuple_desc, note::TITLE)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
            reason: "title is NULL".to_string(),
        })
    })?;

    let content = extract_text(tuple, tuple_desc, note::CONTENT)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let content_hash =
        extract_content_hash(tuple, tuple_desc, note::CONTENT_HASH)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let ttl_str = extract_text(tuple, tuple_desc, note::TTL)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let created_at_ts =
        extract_timestamp(tuple, tuple_desc, note::CREATED_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let updated_at_ts =
        extract_timestamp(tuple, tuple_desc, note::UPDATED_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let accessed_at_ts =
        extract_timestamp(tuple, tuple_desc, note::ACCESSED_AT)?.ok_or_else(|| {
            CaliberError::Storage(StorageError::TransactionFailed {
    ...
    let access_count = extract_i32(tuple, tuple_desc, note::ACCESS_COUNT)?.ok_or_else(|| {
        CaliberError::Storage(StorageError::TransactionFailed {
...
mod tests {
    ...
    mod pg_tests {
        ...
        fn prop_note_insert_get_roundtrip() {
            ...
            let strategy = (arb_note_title(), arb_content(), arb_note_type(), arb_ttl());

            runner
                .run(&strategy, |(title, content, note_type, ttl)| {
        ...
        fn prop_note_update_nonexistent_returns_false() {
            ...
            runner
                .run(&any::<[u8; 16]>(), |bytes| {
                    ...
                    prop_assert!(
                        !result.unwrap(),
                        "Update of non-existent note should return false"
                    );
            ...
            runner
                .run(&any::<[u8; 16]>(), |bytes| {
                    ...
                    prop_assert!(
                        ...
                        "Query for non-existent trajectory should be empty"
                    );
