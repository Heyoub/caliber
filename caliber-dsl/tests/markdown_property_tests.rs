...
/// Creates a proptest Strategy that generates random `InjectionDef` values.
///
/// Generated fields:
/// - `source` and `target`: lowercase underscore names matching `[a-z_]+`.
/// - `mode`: an arbitrary `InjectionMode`.
/// - `priority`: an integer in `0..1000`.
/// - `max_tokens` and `filter`: always `None`.
///
/// # Examples
///
/// ```
/// use proptest::prelude::*;
/// // obtain a single generated value from the strategy
/// let mut runner = proptest::test_runner::TestRunner::default();
/// let strat = crate::arb_injection_def();
/// let value = strat.new_tree(&mut runner).unwrap().current();
/// // `value` is an `InjectionDef` with the described fields
/// ```
...
///
/// ```
/// let strategy = crate::arb_caliber_ast();
/// let mut runner = proptest::test_runner::TestRunner::default();
/// let tree = strategy.new_tree(&mut runner).unwrap();
/// let ast = tree.current();
/// assert_eq!(ast.version, "1.0");
/// assert!(!ast.definitions.is_empty() && ast.definitions.len() <= 3);
/// ```
